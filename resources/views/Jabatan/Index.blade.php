...
@section('dashboard')
    <div class="p-2">
        <h1 class="font-bold text-2xl my-2 ">Jabtan</h1>
        <div class="flex flex-col w-full bg-white shadow-md px-3 py-1 mb-10 mt-3">
            <a class=" mt-3 rounded-md bg-primary p-2 text-white" href="{{ route('jabatan.add') }}">Tambahkan data</a>
            <div class=" flex my-3">
                ...
                <tbody>
                    ...
                    @foreach ($jabatan as $item)
                        <tr>
                            ...
                            <td>{{ $item->name }}</td>
                            <td>{{ $item->gaji }}</td>
                            <td><a href="{{ route('jabatan.edit', ['id' => $item->id]) }}"><svg
            ...
            <div class=" flex w-full pl-3  pt-3 border-t border-blue-950">
                <p>Halaman {{ $jabatan->currentPage() }} Dari {{ $jabatan->lastPage() }} </p>
                <div class="ml-auto flex text-lg font-bold">

                    <a
                        @if ($jabatan->previousPageUrl()) class="mr-10 " href="{{ $jabatan->previousPageUrl() }}&pagination={{ $page }}&search={{ $search }}"
                    @else class="mr-10 text-slate-300" @endif>
                        < </a>

                            <a
                                @if ($jabatan->nextPageUrl()) class="mr-10 " href="{{ $jabatan->nextPageUrl() }}&pagination={{ $page }}&search={{ $search }}" @else class="mr-10 text-slate-300" @endif>>
                            </a>
                </div>
            </div>
        </div>
