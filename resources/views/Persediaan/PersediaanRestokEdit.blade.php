        ...
        <div class=" bg-white rounded-sm p-6 rounded-md shadow-md w-fit">
            <p class=" ">
                ...
                <span class=" font-bold">
                    {{ $riwayat->persediaan->jumlah }}
                </span>
                {{ $riwayat->persediaan->satuan }}
            </p>
            <form action="" method="post" enctype="multipart/form-data">
                @csrf
                <div class=" flex items-center mt-3">
                    <label for="jumlah" class="block w-36">Jumlah</label>
                    <input type="text" placeholder="" name="jumlah" id="jumlah" value="{{ $riwayat->jumlah }}"
                        class=" rounded-sm w-80 block px-2 border border-slate-300 placeholder:text-slate-300">
                    <label for="jumlah" class="block w-36 ml-2">{{ $riwayat->persediaan->satuan }}</label>
                </div>
                @error('jumlah')
                ...
                <div class=" flex items-center mt-3">
                    ...
                    <input type="text" name="nominal" id="nominal" value="{{ $riwayat->nominal }}"
                        class=" rounded-sm w-80 block px-2 border border-slate-300 placeholder:text-slate-300">
                </div>
                @error('nominal')
                    <span class=" ml-36 text-[10px] text-red-500">{{ $message }}</span>
